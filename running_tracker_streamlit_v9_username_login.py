...
def username_to_internal_email(username: str) -> str:
    ...
    return f"{username}@running-tracker.local"

def get_user_display_name(profile=None):
...
def auth_screen():
    ...
    st.markdown(
        ...
        'Create an account or sign in with your username to track your running progress, body weight, pace, calories, streaks, '
    ...
    st.info(
        "Your running and body-weight data are stored securely in the app database and are only visible to your account. "
        "The app administrator may access database records for maintenance purposes."
    )
    ...
    with login_tab:
        ...
        if submitted:
            try:
                ...
                res = supabase.auth.sign_in_with_password({"email": internal_email, "password": password})
    ...
    with signup_tab:
        with st.form("signup_form"):
            username = st.text_input(
                "Username",
                key="signup_username",
                help="Use letters, numbers, underscores, or dots. Example: akbar_runner"
            )
        ...
        if submitted:
            try:
                ...
                supabase = get_supabase()
                internal_email = username_to_internal_email(username)
                res = supabase.auth.sign_up({"email": internal_email, "password": password})
                ...
                if res.user and res.session:
                    ...
                    st.info("Account created. You can now log in with your username.")
            except Exception as e:
...
with st.sidebar:
    st.header("👤 Account")
    st.write(f"Signed in as **{get_user_display_name(profile)}**")
    st.caption("Username-based account")
    ...
    username = str(profile.get("username", "") or get_user_display_name(profile))
    ...
    if df.empty:
        ...
        with row1_col2:
            if not dfp["weight_kg"].dropna().empty:
                ...
                st.markdown('<div class="chart-title">Body Weight Trend</div>', unsafe_allow_html=True)
                st.info("Add body-weight entries to view this chart.")
        ...
        with row3_col2:
            ...
            if not run_df.empty:
                ...
                st.info("Add running entries to view this chart.")
